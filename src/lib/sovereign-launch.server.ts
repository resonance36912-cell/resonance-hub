...
export async function prepareSovereignLaunch(request: Request, app: LaunchApp): Promise<Response> {
  if (process.env.RONS_SOVEREIGN_PROXY_ENABLED !== "1") return Response.json({ error: "Not found" }, { status: 404 });
  ...
  let secret: string;
  try { secret = await exchangeSecret(); } catch { return Response.json({ error: "Launch service unavailable" }, { status: 503 }); }
  const gateway = (process.env.RESONANCE_SOVEREIGN_GATEWAY_URL ?? GATEWAY).replace(/\/$/, "");
  const broker = (process.env.RONS_LAUNCH_BROKER_URL ?? BROKER).replace(/\/$/, "");
  let issued: Response;
  try {
    issued = await fetch(`${gateway}/v1/auth/launch-ticket`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${session}`, "X-RONS-Exchange-Key": secret },
      body: JSON.stringify({ app }),
    });
  } catch { return Response.json({ error: "Launch service unavailable" }, { status: 503 }); }
  if (!issued.ok) return Response.json({ error: issued.status === 401 ? "Authentication required" : "Launch denied" }, { status: issued.status });
  const issueBody = await issued.json() as { ticket?: string };
  if (!issueBody.ticket) return Response.json({ error: "Invalid launch response" }, { status: 502 });
  ...
  } catch { return Response.json({ error: "Launch broker unavailable" }, { status: 503 }); }
  if (!prepared.ok) return Response.json({ error: "Launch preparation failed" }, { status: prepared.status });
  const body = await prepared.json() as { code?: string };
  if (!body.code || body.code.length > 512) return Response.json({ error: "Invalid launch preparation" }, { status: 502 });
