...
export const ProjectMemberRole = z.enum(["collaborator", "reviewer", "observer"]);
  ...
  for (const key of Object.keys(value)) {
    if (FORBIDDEN_METADATA_KEY.test(key)) {
      ctx.addIssue({
        code: "custom",
        path: [key],
        message: "credential_metadata_forbidden",
      });
    }
  }
}
...
export const AttachIntegrationInput = z
  .object({
    ...
    metadata: z
      .record(z.string(), IntegrationMetadataValue)
      .optional()
      .default({}),
  })
  .strict()
  .superRefine((value, ctx) => {
    ...
    if (FORBIDDEN_EXTERNAL_REF.test(value.external_ref)) {
      ctx.addIssue({
        code: "custom",
        path: ["external_ref"],
        message: "credential_reference_forbidden",
      });
    }
    if (
