...
export function parseImportJsonl(text: string, defaultSourceKey: string): ImportNormalizationResult {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    try {
      ...
      const identity = [envelope.source_key, envelope.external_id, envelope.content].join(String.fromCharCode(0));
    ...
    for (const [mappingKey, rawNode] of Object.entries(mapping)) {
      ...
      try {
        ...
        envelopes.push({
          ...
          external_id: ["chatgpt", conversationId, messageId, mappingKey].join(":"),
