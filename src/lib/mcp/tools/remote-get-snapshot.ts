...
export default defineTool({
  ...
  description:
    "Read the latest report-only snapshot from an enrolled PC. Device-reported information is not an independent attestation.",
  ...
  handler: async ({ device_id }, ctx) => {
    ...
    const result = await callBridge(token, { action: "get", device_id });
    return {
