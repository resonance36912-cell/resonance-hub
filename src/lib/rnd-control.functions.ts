...
import { githubJson } from "@/lib/github-provider.server";
...
async function assertRndAdmin(context: AuthContext) {
  ...
  if (!request) throw new Error("Unauthorized: request context unavailable");
  const credential = resolveRonsRequestCredential(request);
  if (!credential) throw new Error("Unauthorized: session credential unavailable");
  const email = await fetchBackendUserEmail(credential);
  if (!email) throw new Error("Forbidden: authenticated email unavailable");
...
async function getRndControlOperatorId(admin: any): Promise<string> {
...
async function ensureRndControlOperator(admin: any): Promise<string> {
...
async function readMutationControl(admin: any) {
  const { data, error } = await admin
    .from("rnd_control_settings")
    .select("mutations_enabled_until,emergency_lock,recovery_hold,operator_user_id,updated_by,updated_at")
    .eq("singleton", true)
    .maybeSingle();
  if (error) throw new Error(error.message);
...
async function githubGet(path: string): Promise<any> {
  try {
    return await githubJson(path, { method: "GET" });
  } catch (error) {
    return {
      unavailable: true,
...
export const bootstrapRndAgent = createServerFn({ method: "POST" })
  .middleware([requireRonsAuth])
  .validator((value: unknown) => DeviceInput.parse(value))
...
export const queueRndOperation = createServerFn({ method: "POST" })
  .middleware([requireRonsAuth])
  .validator((value: unknown) => QueueInput.parse(value))
  .handler(async ({ data, context }) => {
    const actor = await assertRndAdmin(context as AuthContext);
    const admin = await db();
...
export const approveRndOperation = createServerFn({ method: "POST" })
  .middleware([requireRonsAuth])
  .validator((value: unknown) => JobInput.parse(value))
  .handler(async ({ data, context }) => {
    const actor = await assertRndAdmin(context as AuthContext);
    const admin = await db();
    const operatorUserId = await getRndControlOperatorId(admin);
    ...
    if (
      ...
      job.payload?.human_actor_user_id !== context.userId ||
      job.payload?.human_actor_email !== actor.email ||
...
export const setRndMutationWindow = createServerFn({ method: "POST" })
  .middleware([requireRonsAuth])
  .validator((value: unknown) => MutationWindowInput.parse(value))
  .handler(async ({ data, context }) => {
    const actor = await assertRndAdmin(context as AuthContext);
    const admin = await db();
    const operatorUserId = await getRndControlOperatorId(admin);
