/**
 * POST /api/public/usage/release
 *   Authorization: Bearer <supabase_access_token>
 *   Body: { reservationId: uuid, reason?: string (max 200) }
 *
 * Cancels a prior reservation on failure: refunds the wallet and marks the
 * reservation `released` with the provided reason stored in metadata. No-op
 * (returns the row as-is) if the reservation is already terminal — safe to
 * retry from a spoke's error handler.
 *
 * The caller MUST own the reservation or the request is rejected 403.
 *
...
export const Route = createFileRoute("/api/public/usage/release")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        const { data: owner, error: ownerErr } = await supabaseAdmin
          ...
          .maybeSingle();
        if (ownerErr) {
          console.error("[usage/release] ownership lookup failed:", ownerErr);
        ...
        const { data, error } = await supabaseAdmin.rpc("release_reservation", {
          ...
          _reason: body.reason ?? "released",
        });
        if (error) {
          console.error("[usage/release] rpc failed:", error);
          return usageJson({ error: "release_failed", message: error.message }, 500);
        }
