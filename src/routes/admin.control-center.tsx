...
export const Route = createFileRoute("/admin/control-center")({
  head: () => ({ meta: [{ title: "RONS Control Center — Resonance Admin" }, { name: "robots", content: "noindex, nofollow" }] }),
  beforeLoad: async () => {
...
function ControlCenter() {
  ...
  const q = useQuery({ queryKey: ["rons-control-state"], queryFn: () => fetchState(), refetchInterval: 5000 });
  const d = q.data;
  ...
  const council = useMutation({ mutationFn: () => compare({ data: { prompt, system: "Apply RCGF v1.0. Analyze this RONS optimization request. Return recommendations only; no production authority.", providers: selectedProviders, human_approved_external: approveExternalRun } }), onSuccess: () => setApproveExternalRun(false) });
  const promote = useMutation({ mutationFn: (r: CouncilResult) => promoteRecommendation({ data: { title: `AI Council: ${String(r.provider)}`, rationale: String(r.text ?? ""), target_scope: "rons", provider: String(r.provider), model: String(r.model ?? ""), receipt_id: String(r.receipt_id ?? ""), prompt } }) });
  return <div className="min-h-screen bg-background text-foreground">
    <div className="mx-auto max-w-7xl px-6 py-12 space-y-10">
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-xs uppercase tracking-[0.3em] text-muted-foreground">Resonance Admin · RONS</p>
          <h1 className="mt-2 text-3xl font-semibold">AI Development & Governance Control Center</h1>
          <p className="mt-2 max-w-3xl text-sm text-muted-foreground">Governed optimization across local RONS and approved external AI providers, with cost visibility, promotion assets, and human approval gates.</p></div>
        <Link to="/admin" className="rounded-lg border border-border px-4 py-2 text-sm hover:bg-accent">← Admin</Link>
      </header>

      {q.isLoading && <p className="text-muted-foreground">Loading control plane…</p>}
      {q.error && <div className="rounded-xl border border-red-500/40 bg-red-500/10 p-4 text-red-300">{(q.error as Error).message}</div>}
      {d && <>
        <section className="grid md:grid-cols-4 gap-3">
          <Card label="Governance" value={d.governance.framework} sub="Human sovereignty: enforced" />
          <Card label="Local AI" value={(d.localHealth as LocalHealth).ok ? "ONLINE" : "OFFLINE"} sub={String((d.localHealth as LocalHealth).model ?? "RONS local model")} />
          <Card label="External AI" value="APPROVAL GATED" sub="Disabled by default" />
          <Card label="Cost feed" value="5s refresh" sub={`Updated ${new Date(d.generatedAt).toLocaleTimeString()}`} />
        </section>

        <section className="rounded-xl border border-border bg-card p-5">
          <h2 className="text-lg font-semibold">Resonance Governance Protocol</h2>
          <div className="mt-3 flex flex-wrap gap-2">{d.governance.workflow.map((s, i) => <span key={s} className="rounded-full border border-border bg-background px-3 py-1 text-xs">{i + 1}. {s}</span>)}</div>
          <p className="mt-4 text-sm text-muted-foreground">External providers, governance changes, production deployment, destructive database changes, secret rotation and network enablement remain human-approved actions. Receipts are append-only.</p>
        </section>
        <section>
          <div className="mb-3 flex items-end justify-between gap-3"><div><h2 className="text-lg font-semibold">AI provider cost board</h2><p className="text-sm text-muted-foreground">Rates are separated from historical usage receipts; local RONS remains zero-credit.</p></div><span className="text-xs text-muted-foreground">USD token rates</span></div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">{d.providers.map((p) => <div key={p.id} className="rounded-xl border border-border bg-card p-5">
            <div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{p.name}</p><p className="mt-1 text-xs text-muted-foreground">{p.note}</p></div><span className={`rounded-full px-2 py-1 text-[10px] uppercase tracking-wide ${p.enabled ? "bg-emerald-500/15 text-emerald-300" : "bg-amber-500/15 text-amber-300"}`}>{p.enabled ? "enabled" : "approval required"}</span></div>
            <div className="mt-4 grid grid-cols-2 gap-2 text-sm"><div className="rounded-lg bg-background p-3"><p className="text-xs text-muted-foreground">Input</p><p className="mt-1 font-mono">{usd(p.inputUsdM)}</p></div><div className="rounded-lg bg-background p-3"><p className="text-xs text-muted-foreground">Output</p><p className="mt-1 font-mono">{usd(p.outputUsdM)}</p></div></div>
          </div>)}</div>
        </section>
