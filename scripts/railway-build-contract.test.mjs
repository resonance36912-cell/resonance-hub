...
// Dependency-free configuration contract tests, NOT a Vite/Nitro build.
// Imports are replaced with plugin stubs; plugin execution and application
// routes/authentication still require the real build and integration tests.
...
test("cloud auth-shadow definition is off even with inherited local flags", () => {
  ...
  assert.deepEqual(Object.keys(config.define), ["import.meta.env.VITE_RONS_AUTH_SHADOW"]);
});

test("Node output is isolated and ignores RONS_LOCAL_OUTPUT_DIR", () => {
  const config = inspectConfig({ RONS_LOCAL_OUTPUT_DIR: ".output-local" });
  assert.deepEqual(plain(options(config, "nitro")), {
    preset: "node-server", output: { dir: ".output-railway" },
  });
});
...
test("server-only import protection remains an error", () => {
  ...
  assert.deepEqual(plain(start.importProtection), {
    behavior: "error", client: { files: ["**/server/**"], specifiers: ["server-only"] },
  });
});

test("the cloud target contains only the intended five plugins", () => {
...
test("Railway build runs contract tests and the existing commercial-mode prebuild", () => {
  assert.equal(pkg.scripts["build:railway"],
...
test("the existing local build and start commands remain unchanged", () => {
  assert.equal(pkg.scripts["build:local"],
    "bun run scripts/assert-local-build-target-safe.ts && vite --config vite.local.config.ts build");
