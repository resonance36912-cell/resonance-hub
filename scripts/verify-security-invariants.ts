 ...
 *  3. No `process.env.*SERVICE_ROLE*` access outside server-only files.
 *  4. No `dangerouslySetInnerHTML` outside an allowlist.
 *  5. No `eval(` or `new Function(` calls in the codebase.
 *  6. Public API routes (`src/routes/api/public/**`) must mention either
 *     `signature` verification, `has_role`, or `requireSupabaseAuth`
