...
for (const key of Object.keys(APP_REGISTRY) as ResonanceAppKey[]) {
  ...
  // Meta description follows the status-aware registry contract.
  ...
  if (!desc) {
    ...
    fail(
      `Meta description matches registry contract (${path})`,
      `got ${JSON.stringify(desc)}, want ${JSON.stringify(expectedDescription)}`,
    );
  } else {
    pass(`Meta description matches registry contract (${path})`);
  }
