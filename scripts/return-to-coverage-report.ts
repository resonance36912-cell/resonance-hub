...
<style>
  ...
  body { font-family: ui-sans-serif, system-ui, -apple-system, "Segoe UI", Roboto, sans-serif;
         margin: 0; padding: 32px; color: #10121a; background: #fff; }
  ...
  footer { margin-top: 32px; font-size: 11px; color: #7a8092; }
</style></head>
<body>
  ...
  ${green ? `<p class="sub" style="margin-top:24px">No counterexamples were produced — every fuzzed, normalized and percent-encoded input resolved to an allowlisted origin or a safe Hub fallback.</p>` : failures}
...
lines.push(
  `${pad("SUITE", 34)}${pad("PASS", 7)}${pad("FAIL", 7)}${pad("ASSERTIONS", 12)}STATUS`,
);
for (const r of results) {
  lines.push(
    `${pad(r.title, 34)}${pad(String(r.pass), 7)}${pad(String(r.fail), 7)}${pad(
...
lines.push(
  `${pad("TOTAL", 34)}${pad(String(totals.pass), 7)}${pad(String(totals.fail), 7)}${pad(
