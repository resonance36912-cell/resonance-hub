...
describe("DataNest collaboration schema", () => {
  test("has exactly one CLI-generated collaboration migration", () => {
    expect(migrationNames).toHaveLength(1);
  });

  test("reuses Nova projects and bridge devices instead of duplicate authorities", () => {
  ...
  test("enables RLS and keeps collaboration tables server-only", () => {
    ...
    expect(sql).not.toMatch(
      /GRANT SELECT ON TABLE public\.datanest_project_(integrations|devices) TO authenticated/,
    );
  });
});
