...
describe("DataNest/Nova MCP governed boundary", () => {
  test("pins the validated MCP runtime and keeps governed endpoints source-owned", async () => {
    expect(packageJson.dependencies?.["@lovable.dev/mcp-js"]).toBe("0.20.0");
    expect(viteSource).not.toContain('@lovable.dev/mcp-js/stacks/tanstack/vite');
    expect(viteSource).not.toContain("mcpPlugin()");
    ...
    for (const path of [
      ...
      const routeSource = await Bun.file(path).text();
      expect(routeSource).toContain("RONSAS-owned governed MCP route");
      expect(routeSource).toContain("@lovable.dev/mcp-js/stacks/tanstack");
    }
  });
