...
/** Runner output can carry ANSI colour codes (vitest); they print as tofu boxes. */
export function stripAnsi(v: string): string {
  // eslint-disable-next-line no-control-regex
  return v.replace(/\u001B\[[0-9;]*[A-Za-z]/g, "");
}
...
<body>
  ...
  <h2>Runner output</h2>
  ${outputSection}
  <pre>${esc(stripAnsi(s.output) || "(no output captured)")}</pre>
